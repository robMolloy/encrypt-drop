...
export const CompletionPages = (p: { status: keyof typeof STATUS_CONTENT_MAP }) => {
  const intentId = "intentId";
  return (
    <div id="payment-status">
      <div id="status-icon" style={{ backgroundColor: STATUS_CONTENT_MAP[p.status].iconColor }}>
        {STATUS_CONTENT_MAP[p.status].icon}
      </div>
      <h2 id="status-text">{STATUS_CONTENT_MAP[p.status].text}</h2>
      {intentId && (
...
export default function PaymentCompletePage() {
  return (
    <main className={`min-h-screen`}>
      <Typography fullPage>
        <div>
          <CompletionPages status={"succeeded"} />
        </div>
      </Typography>
    </main>
  );
